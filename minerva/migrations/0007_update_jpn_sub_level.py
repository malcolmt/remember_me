...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        filename = os.path.join(os.path.dirname(__file__), "jpn_update.json.gz")
        for item in json.loads(gzip.open(filename).read()):
            orm.Word.objects.filter(language="jpn", word=item["word"]). \
                    update(sub_level=item["sub_level"])

    def backwards(self, orm):
        orm.Word.objects.filter(language="jpn").update(sub_level=None)
